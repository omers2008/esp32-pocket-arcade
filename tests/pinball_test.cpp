...
int main() {
  ...
  p.leftHeld = true; p.ballX = 17; p.ballY = 52; p.ballVx = 0; p.ballVy = 1;
  uint32_t oldScore = p.score; p.stepBall(); assert(p.score > oldScore && p.ballVy < 0);

  p.ballX = 27; p.ballY = 65; p.ballVy = 1; p.stepBall(); assert(p.lives == 2 && p.waiting && !p.over);
  p.update(false, false); p.update(false, true); assert(!p.waiting && p.ballVy < 0);
  p.lives = 1; p.ballX = 27; p.ballY = 65; p.ballVx = 0; p.ballVy = 1; p.stepBall(); assert(p.over && !p.won);
